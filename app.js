...
(function () {
  ...
  const STR = {
    en: {
      ...
      ac_guide: "ℹ️ About this chapter",
      ac_chapter: "Chapter",
      ac_ch1_name: "Flow basics",
      ac_ch2_name: "Pressure & forces",
      ac_ch3_name: "Efficiency & design",
      ac_ch4_name: "Aerodynamics in practice",
      ac_ch5_name: "The vortex street",
      ch1_guide: "<p>This chapter builds the three ideas everything else rests on: how the flow behaves, what slows a body down, and what pushes it sideways.</p><ul><li><strong>Reynolds number</strong> = speed × size ÷ viscosity. Low means smooth, syrupy flow; high means the wake breaks into vortices. Raise the speed or lower the viscosity and watch the wake.</li><li><strong>Drag</strong> is the push along the wind. A blunt body leaves a fat wake and drags hard; a slender one slips through — watch the drag bar.</li><li><strong>Lift</strong> is the sideways push. Tilt a wing: it deflects air one way and is pushed the other. Watch the lift arrow.</li><li><strong>Step by step:</strong> change one slider at a time and see which readout moves — that is how the intuition sticks.</li></ul>",
      ch2_guide: "<p>Forces are just air pressure pushing on every part of the surface. Switch the field to <strong>Pressure</strong> to see where the pushes are.</p><ul><li><strong>Read the map:</strong> bright = high pressure (air piling up — the stagnation point sits on the nose); dark = low pressure (air sped up). The force is the imbalance between them.</li><li><strong>Frontal area:</strong> a flat plate face-on has the largest front, a huge high-pressure zone ahead and suction behind — the biggest drag of any shape.</li><li><strong>Downforce:</strong> flip a wing over and the imbalance points down — that is how a race car is pressed onto the track. Same physics as lift, upside down.</li><li><strong>Step by step:</strong> try to predict a shape from its pressure map before looking at the force bars.</li></ul>",
      ch3_guide: "<p>Making lift is easy; making it <strong>cheaply</strong> is the engineer's job. This chapter is about the balance between what you want (lift) and what it costs (drag).</p><ul><li><strong>Lift-to-drag ratio (L/D)</strong> is the score that matters. A gentle angle lifts a lot for little drag; too steep and drag balloons for barely more lift.</li><li><strong>Find the sweet spot:</strong> sweep the angle slowly and watch L/D climb then fall. Real gliders and airliners cruise right there.</li><li><strong>Design your own:</strong> draw a smooth, slender body and try to beat the presets on drag. Bluff is bad, streamlined is good.</li><li><strong>Step by step:</strong> the best shape is not the one with the most lift — it is the one with the best balance.</li></ul>",
      ch4_guide: "<p>Now put it together like a real designer: hit two targets at once, weigh a trade-off, and build a wing from scratch.</p><ul><li><strong>Cruise wing:</strong> it must lift enough AND stay efficient. Tune the angle until the lift and the L/D readouts are both strong at the same time.</li><li><strong>Downforce trade-off:</strong> every extra degree adds drag. Find the most downforce you can get before the drag runs away.</li><li><strong>Your own wing:</strong> asymmetry makes lift. A symmetric shape makes none — arch the top so the flow bends down and the shape is pushed up. Flip the curve if the arrow points the wrong way.</li><li><strong>Step by step:</strong> there is no single right number — good design is hitting the target with margin.</li></ul>",
      ch5_guide: "<p>The finale, all in the <strong>Vortices</strong> view — the most dramatic thing a flow does, and why it matters for real structures.</p><ul><li><strong>Kármán vortex street:</strong> behind a blunt body at high Reynolds the wake cannot stay still — it sheds vortices alternately left and right in a steady rhythm. Raise the speed or lower the viscosity and watch it build.</li><li><strong>Why it matters:</strong> that rhythm is a periodic sideways force — it makes chimneys hum and cables and bridge decks vibrate (vortex-induced vibration).</li><li><strong>Streamlining kills it:</strong> a slender body lets the flow close in gently, so no vortices form and the vibration is gone — at the same speed. Shape, not speed, does it.</li><li><strong>Step by step:</strong> watch the 🌀 number — it is the strength of the street. Chunky and blunt roars; thin and streamlined stays calm.</li></ul>",
    ...
    pl: {
      ...
      ac_guide: "ℹ️ O rozdziale",
      ac_chapter: "Rozdział",
      ac_ch1_name: "Podstawy przepływu",
      ac_ch2_name: "Ciśnienie i siły",
      ac_ch3_name: "Sprawność i projekt",
      ac_ch4_name: "Aerodynamika w praktyce",
      ac_ch5_name: "Ulica wirowa",
      ch1_guide: "<p>Ten rozdział buduje trzy idee, na których opiera się cała reszta: jak zachowuje się przepływ, co spowalnia ciało i co pcha je w bok.</p><ul><li><strong>Liczba Reynoldsa</strong> = prędkość × rozmiar ÷ lepkość. Mała to gładki, gęsty przepływ; duża to ślad rozpadający się na wiry. Zwiększ prędkość albo zmniejsz lepkość i patrz na ślad.</li><li><strong>Opór</strong> to pchnięcie wzdłuż wiatru. Tępe ciało zostawia szeroki ślad i mocno hamuje; smukłe prześlizguje się — patrz na pasek oporu.</li><li><strong>Siła nośna</strong> to pchnięcie w bok. Przechyl skrzydło: odchyla powietrze w jedną stronę, a samo jest pchane w drugą. Patrz na strzałkę nośnej.</li><li><strong>Krok po kroku:</strong> zmieniaj jeden suwak naraz i patrz, który odczyt się rusza — tak utrwala się intuicja.</li></ul>",
      ch2_guide: "<p>Siły to po prostu ciśnienie powietrza naciskające na każdy fragment powierzchni. Przełącz pole na <strong>Ciśnienie</strong>, żeby zobaczyć, gdzie są naciski.</p><ul><li><strong>Odczytaj mapę:</strong> jasne = wysokie ciśnienie (powietrze się piętrzy — punkt stagnacji na dziobie); ciemne = niskie ciśnienie (powietrze przyspieszyło). Siła to różnica między nimi.</li><li><strong>Pole czołowe:</strong> płaska płyta na wprost ma największy przód, ogromną strefę wysokiego ciśnienia z przodu i podciśnienie z tyłu — największy opór ze wszystkich kształtów.</li><li><strong>Docisk:</strong> odwróć skrzydło, a różnica ciśnień skieruje się w dół — tak dociska się bolid do toru. Ta sama fizyka co nośna, do góry nogami.</li><li><strong>Krok po kroku:</strong> spróbuj odgadnąć kształt z jego mapy ciśnienia, zanim spojrzysz na paski sił.</li></ul>",
      ch3_guide: "<p>Wytworzyć nośną jest łatwo; wytworzyć ją <strong>tanio</strong> — to zadanie inżyniera. Ten rozdział jest o równowadze między tym, czego chcesz (nośna), a tym, ile to kosztuje (opór).</p><ul><li><strong>Stosunek nośnej do oporu (L/D)</strong> to wynik, który się liczy. Łagodny kąt daje dużo nośnej za mały opór; za stromo — opór puchnie przy ledwie większej nośnej.</li><li><strong>Znajdź sweet-spot:</strong> przemiataj kąt powoli i patrz, jak L/D rośnie, a potem spada. Szybowce i samoloty pasażerskie lecą właśnie tam.</li><li><strong>Zaprojektuj własny:</strong> narysuj gładki, smukły kształt i spróbuj pobić presety na oporze. Tępe — źle, opływowe — dobrze.</li><li><strong>Krok po kroku:</strong> najlepszy kształt to nie ten z największą nośną — to ten z najlepszą równowagą.</li></ul>",
      ch4_guide: "<p>Teraz złóż to jak prawdziwy projektant: trafiaj w dwa cele naraz, waż kompromis i zbuduj skrzydło od zera.</p><ul><li><strong>Skrzydło krążownicze:</strong> musi dawać dość nośnej I być sprawne. Dobierz kąt tak, by odczyt nośnej i L/D były jednocześnie mocne.</li><li><strong>Kompromis docisku:</strong> każdy dodatkowy stopień dokłada oporu. Znajdź maksimum docisku, zanim opór ucieknie.</li><li><strong>Własne skrzydło:</strong> nośna bierze się z asymetrii. Kształt symetryczny nie daje nic — wygnij górę, by przepływ zszedł w dół, a kształt został pchnięty w górę. Odwróć wygięcie, jeśli strzałka wskazuje źle.</li><li><strong>Krok po kroku:</strong> nie ma jednej dobrej liczby — dobry projekt to trafienie w cel z zapasem.</li></ul>",
      ch5_guide: "<p>Finał, cały w widoku <strong>Wiry</strong> — najbardziej dramatyczna rzecz, jaką robi przepływ, i dlaczego liczy się dla prawdziwych konstrukcji.</p><ul><li><strong>Ulica wirowa Kármána:</strong> za tępym ciałem przy dużej liczbie Reynoldsa ślad nie może ustać — odrywa wiry naprzemiennie w lewo i w prawo, w równym rytmie. Zwiększ prędkość albo zmniejsz lepkość i patrz, jak się buduje.</li><li><strong>Dlaczego to ważne:</strong> ten rytm to okresowa siła boczna — sprawia, że kominy buczą, a liny i pomosty mostów drgają (drgania wzbudzane wirami).</li><li><strong>Opływowość to zabija:</strong> smukłe ciało pozwala przepływowi zamknąć się łagodnie, więc wiry nie powstają, a drgania znikają — przy tej samej prędkości. Robi to kształt, nie prędkość.</li><li><strong>Krok po kroku:</strong> patrz na liczbę 🌀 — to siła ulicy wirowej. Przysadziste i tępe ryczy; cienkie i opływowe pozostaje spokojne.</li></ul>",
  ...
  function wire() {
    ...
    el("ah-guide").addEventListener("click", () => {
      const lv = ACADEMY[academy.level]; if (!lv) return;
      qEl("chapter-title").textContent = t("ac_chapter") + " " + lv.ch + " — " + t("ac_ch" + lv.ch + "_name");
      qEl("chapter-body").innerHTML = t("ch" + lv.ch + "_guide");
      if (chapterModal && typeof chapterModal.showModal === "function") chapterModal.showModal();
    });
